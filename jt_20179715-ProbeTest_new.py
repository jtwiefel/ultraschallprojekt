...
RawData = "rawdata/probeX_new.data"
f = open(RawData, "r")
...
#FFT is probably used to filter unnecessary frequencies
...
Scale = max(FFT)
ff = [ Fech*2.0*x/(2.0*len(V)) for x in range(len(V))]
...
#divided by 2 because it is mirrored at the Y axis
#use scale to normalize between 0 and 1
...
#transform the cleaned signal back to time 
F = np.real(np.fft.ifft(FFTCleaned))

#cuts off everything below zero
FH = np.asarray(np.abs(signal.hilbert(F)))

plt.figure(figsize=(15,5))


plt.plot(range(len(rawSig)),rawSig,"g")
plt.plot(range(len(F)),F,"b")
plt.plot(range(len(FH)),FH,"r")
#plt.savefig('Imgs/fft_'+RawData.split("/")[-1]+".png", bbox_inches='tight')
plt.show()
...
num_adc = 2
tableData = np.asarray(FH).reshape((1000,num_adc*2500))
tableDataRaw = np.asarray(rawSig).reshape((1000,num_adc*2500))
...
#fill first 400 with background color
tableData = np.asarray(FH).reshape((1000,num_adc*2500))
Offset = 400
